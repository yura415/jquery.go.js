...
phantom.create("--web-security=false", "--ignore-ssl-errors=true", "--ssl-protocol=any", function(ph) {
  ...
  // Empty the get instance queue.
  _.each(instanceQueue, function(queue) {
    queue(instance);
  });
...
var jQuery = _.extend(function(selector, context) {
  ...
  /**
   * Include's jQuery to page if "jQuery" is undefined
   * @param {function} callback
   *   Called when the jQuery script is included.
   */
  ensureJqueryIsLoaded: function (callback) {
    var self = this;
      ...
      page.evaluate(function() {
        ...
        var loadJS = function() {
          if (loading) {
            ...
            page.includeJs(self.config.jQuery, function() {
              callback();
              callback = _.identity;
            });
          }
  ...
  visit: function(url, callback) {
    ...
    getPage(function(page) {
      ...
      page.open(self.config.site + url, function(status) {
        ...
        if (self.config.addJQuery) {
          self.ensureJqueryIsLoaded(callback);
        }
        else {
  ...
  waitForPage: function(callback, nowait) {
    ...
    if (nowait) {
      if (loading) {
        ...
        self.ensureJqueryIsLoaded(function() {
          getPage(function(page) {
            page.evaluate(function() {
